...
# 2. Venta promedio semanal: Holiday vs No-Holiday, dentro del periodo filtrado
venta_promedio_periodo = df_filtrado['Weekly_Sales'].mean() if len(df_filtrado) else 0

promedio_no_holiday = df_filtrado.loc[df_filtrado['IsHoliday'] == False, 'Weekly_Sales'].mean()
promedio_holiday = df_filtrado.loc[df_filtrado['IsHoliday'] == True, 'Weekly_Sales'].mean()
...
# 4. Devoluciones (ventas negativas) sobre la venta total del periodo
devoluciones = df_filtrado.loc[df_filtrado['Es_Devolucion'] == True, 'Weekly_Sales'].sum()
pct_devolucion = (abs(devoluciones) / venta_total * 100) if venta_total else 0
