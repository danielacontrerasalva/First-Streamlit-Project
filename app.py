...
MESES_ABREV_ES = {1:'Ene',2:'Feb',3:'Mar',4:'Abr',5:'May',6:'Jun',
...
mes_sel = st.sidebar.multiselect(
    ...
    format_func=lambda m: MESES_ABREV_ES.get(int(m), str(m))
)
...
kpi_card(c4, "QoQ Sales Growth", "kpi-coral", f"{qoq:+.1f}%", f"{trim_label} vs prior Q")
...
if not df_filtrado.empty:
    ...
    etiquetas_periodo = {p: f"{MESES_ABREV_ES.get(p.month, p.month)} {p.year}" for p in periodos_disponibles}
