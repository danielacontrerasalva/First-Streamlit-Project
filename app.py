...
<style>
    ...
    /* El contenedor de alto fijo (st.container(height=...)) dibuja su propio borde
       gris por defecto; lo neutralizamos para que solo se vea la tarjeta verde de
       adentro, y mantenemos overflow:hidden para que el alto fijo se respete. */
    ...
    /* Intento final y más agresivo: cubre TODA etiqueta (no solo div) dentro del
       slider, tanto background (por si es HTML/CSS) como fill/stroke (por si el
       thumb o el track están dibujados con SVG en vez de con CSS background). */
...
ALTO_TARJETA_INSIGHT = 210  # px — fijo para que ambas tarjetas midan siempre lo mismo
...
with col_i1:
    ...
    with st.container(height=ALTO_TARJETA_INSIGHT):
