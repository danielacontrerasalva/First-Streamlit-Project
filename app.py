...
<style>
    ...
    /* Quitar TODO rastro de caja/borde alrededor de st.form (Streamlit lo dibuja
       por defecto con borde + fondo + sombra) para que se sienta parte natural
       del sidebar, no una tarjeta aparte. Solo debe quedar visible el botón. */
    ...
    /* La regla anterior pintaba TODO dentro del slider (incluido el contenedor
       grande que envuelve el widget completo), por eso quedó un rectángulo verde
       sólido. Ahora solo pintamos los elementos "hoja" (sin hijos adentro), que son
       las piezas reales del track/thumb, sin tocar los divs que envuelven todo. */
    /* Enfoque distinto: en vez de adivinar qué elemento interno pinta cada parte
       (texto, track, thumb), aplicamos un filtro de color a nivel de PIXEL sobre
       todo el widget. primaryColor lila (#B9A7F0) queda rotado a verde de paleta,
       sin importar si el color viene de CSS, SVG o lo que sea por dentro. */
    div[data-testid="stSlider"] {
        filter: hue-rotate(-112deg) saturate(0.65);
    }
</style>
...
with st.sidebar.form("filtros_form"):
    ...
    anio_sel = st.multiselect("Year", anios_disponibles, default=anios_disponibles)
    ...
    tipo_tienda_sel = st.multiselect("Store type", tipos_disponibles, default=tipos_disponibles)

    tiendas_disponibles = sorted(df['Store'].unique())
    tienda_sel = st.multiselect("Store (optional)", tiendas_disponibles)

    incluir_holiday = st.checkbox("Include holiday weeks", value=True)
    excluir_devoluciones = st.checkbox("Exclude returns (negative sales)", value=False)

    st.form_submit_button("Apply filters", width='stretch')

if not anio_sel:
    anio_sel = anios_disponibles  # deseleccionar todo = ver todo, no ver nada
if not mes_sel:
    mes_sel = meses_disponibles  # deseleccionar todo = ver todo, no ver nada
if not tipo_tienda_sel:
    tipo_tienda_sel = tipos_disponibles  # deseleccionar todo = ver todo, no ver nada

# ==========================================================
# APLICAR FILTROS
# ==========================================================
# Cacheado por combinación de filtros: si dos reruns (o dos usuarios distintos
# viendo la app al mismo tiempo) piden exactamente la misma combinación, se
# reutiliza el resultado en vez de volver a escanear las 421K filas desde cero.
# El "_df" con guion bajo le dice a Streamlit que no intente hashear el
# DataFrame completo en cada llamada (ya es estable, viene de otro cache).
@st.cache_data
def filtrar_datos(_df, anio_sel, mes_sel, tipo_tienda_sel, tienda_sel, incluir_holiday, excluir_devoluciones):
    df_filtrado = _df[
        _df['Anio'].isin(anio_sel) &
        _df['Mes'].isin(mes_sel) &
        _df['Type'].isin(tipo_tienda_sel)
    ]
    if tienda_sel:
...
df_filtrado = filtrar_datos(
    df, tuple(anio_sel), tuple(mes_sel), tuple(tipo_tienda_sel),
    tuple(tienda_sel), incluir_holiday, excluir_devoluciones
)
...
# Si los filtros del sidebar no dejan ninguna fila (ej. deseleccionar todos los
# meses o todos los tipos de tienda), cortamos aquí con un aviso claro en vez de
# renderizar KPIs en $0, gráficos vacíos y tablas en blanco por toda la página
# — eso se siente como que la app está "bloqueada" aunque técnicamente no truene.
if df_filtrado.empty:
    st.warning("⚠️ No hay datos para la combinación de filtros seleccionada. Ajusta los filtros del panel izquierdo (por ejemplo, selecciona al menos un año, mes y tipo de tienda).")
    st.stop()
...
c1.markdown(
    ...
    'Anchored to the most recent year selected — Month sets the cutoff, not a filter'
    '</div>',
    unsafe_allow_html=True
)
...
c2.markdown('<div style="height:29px;"></div>', unsafe_allow_html=True)
...
c4.markdown(
    '<div style="font-size:11px; color:#7a7a7a; text-align:center; '
    'font-style:italic; margin-top:6px; line-height:1.3;">'
    'Compares weekly average, not total — avoids skew from an incomplete quarter'
    '</div>',
    unsafe_allow_html=True
)
...
# Ambas tarjetas en UN SOLO bloque HTML (flexbox propio, no columnas de Streamlit):
# así controlamos directamente el alto igual y la alineación superior, sin depender
# de cómo Streamlit arma sus divs internos por versión/navegador.
st.markdown(
    f"""
    <div style="display:flex; gap:20px; align-items:stretch; flex-wrap:wrap;">
        <div class="insight-card" style="flex:1 1 320px;">{texto1}</div>
        <div class="insight-card" style="flex:1 1 320px;">{texto2}</div>
    </div>
    """,
    unsafe_allow_html=True
)
...
@st.cache_data
def calcular_tendencia(_df, anio_sel, tipo_tienda_sel, tienda_sel, incluir_holiday, excluir_devoluciones):
    df_para_tendencia = _df[
        _df['Anio'].isin(anio_sel) &
        _df['Type'].isin(tipo_tienda_sel)
    ]
    if tienda_sel:
...
tendencia = calcular_tendencia(
    df, tuple(anio_sel), tuple(tipo_tienda_sel), tuple(tienda_sel),
    incluir_holiday, excluir_devoluciones
)
...
st.markdown(
    '<div style="font-size:11px; color:#7a7a7a; text-align:center; '
    'font-style:italic; margin-top:-8px; margin-bottom:8px; line-height:1.3;">'
    'Always shows the last 12 full months — the Month filter does not narrow this chart'
    '</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    '<div style="font-size:11px; color:#7a7a7a; text-align:center; '
    'font-style:italic; margin-top:-8px; margin-bottom:8px; line-height:1.3;">'
    'A month counts as Holiday if it contains at least one festive week — compares monthly totals, not weekly'
    '</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    '<div style="font-size:11px; color:#7a7a7a; font-style:italic; '
    'margin-top:-8px; margin-bottom:10px; line-height:1.3;">'
    'The range slider below only narrows this table — it doesn\'t affect the KPIs, insights or charts above'
    '</div>',
    unsafe_allow_html=True
)
