...
<style>
    ...
    div[data-testid="stSlider"] div[data-baseweb="tick-bar"] {
...
# insights comparan el promedio semanal por tienda (no la suma total), para
# que un trimestre incompleto o un rango de fechas corto no distorsione la
# comparación entre tiendas o tipos de tienda.
st.subheader("📌 Key insights")
col_i1, col_i2 = st.columns(2)

with col_i1:
    if not df_filtrado.empty:
        ventas_semana_tienda = df_filtrado.groupby(['Store', 'Date'], as_index=False)['Weekly_Sales'].sum()
        promedio_por_tienda = ventas_semana_tienda.groupby('Store')['Weekly_Sales'].mean()
        promedio_general = promedio_por_tienda.mean()
        umbral = promedio_general * 0.8  # 20% por debajo del promedio del grupo filtrado
        tiendas_oportunidad = promedio_por_tienda[promedio_por_tienda < umbral].sort_values()

        if len(tiendas_oportunidad) > 0:
            lista_tiendas = ", ".join(f"#{int(s)}" for s in tiendas_oportunidad.index[:5])
            extra = f" and {len(tiendas_oportunidad) - 5} more" if len(tiendas_oportunidad) > 5 else ""
            st.info(
                f"🎯 **Reinforcement opportunity** — {len(tiendas_oportunidad)} store(s) sell "
                f"more than 20% below the average weekly sales of their peers: {lista_tiendas}{extra}."
            )
        else:
            st.info("🎯 **Reinforcement opportunity** — no store is currently 20%+ below the group average; performance is fairly even.")
    else:
        st.info("🎯 **Reinforcement opportunity** — not enough data for the current filters.")

with col_i2:
    if not df_filtrado.empty:
        ventas_semana_tienda_tipo = df_filtrado.groupby(['Store', 'Type', 'Date'], as_index=False)['Weekly_Sales'].sum()
        promedio_tienda = ventas_semana_tienda_tipo.groupby(['Store', 'Type'])['Weekly_Sales'].mean().reset_index()
        promedio_por_tipo = promedio_tienda.groupby('Type')['Weekly_Sales'].mean().sort_values()

        if len(promedio_por_tipo) >= 2:
            tipo_bajo, tipo_alto = promedio_por_tipo.index[0], promedio_por_tipo.index[-1]
            brecha = (
                (promedio_por_tipo.iloc[-1] - promedio_por_tipo.iloc[0]) / promedio_por_tipo.iloc[-1] * 100
            ) if promedio_por_tipo.iloc[-1] else 0
            st.info(
                f"🏬 **Segment gap** — Type {tipo_bajo} stores average {brecha:.0f}% less weekly sales per store "
                f"than Type {tipo_alto} stores, a possible focus area for store-format strategy."
            )
        else:
            st.info("🏬 **Segment gap** — need at least two store types in the current filters to compare.")
    else:
        st.info("🏬 **Segment gap** — not enough data for the current filters.")
