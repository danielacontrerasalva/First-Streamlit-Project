...
# CÁLCULO DE KPIs
# ==========================================================
# 1. Venta total del periodo filtrado
venta_total = df_filtrado['Weekly_Sales'].sum()

# 2. Venta promedio semanal (todo el periodo) vs. venta promedio semanal
#    calculada solo hasta la fecha del último holiday dentro del periodo filtrado
venta_promedio_periodo = df_filtrado['Weekly_Sales'].mean() if len(df_filtrado) else 0

df_holidays = df_filtrado[df_filtrado['IsHoliday'] == True]
if not df_holidays.empty:
    fecha_ultimo_holiday = df_holidays['Date'].max()
    df_hasta_holiday = df_filtrado[df_filtrado['Date'] <= fecha_ultimo_holiday]
    venta_promedio_hasta_holiday = df_hasta_holiday['Weekly_Sales'].mean() if len(df_hasta_holiday) else 0
    delta_vs_holiday = (
        (venta_promedio_periodo - venta_promedio_hasta_holiday) / venta_promedio_hasta_holiday * 100
    ) if venta_promedio_hasta_holiday else 0
else:
    delta_vs_holiday = 0
...
# 4. Devoluciones / venta bruta
devoluciones = df_filtrado[df_filtrado['Es_Devolucion'] == True]['Weekly_Sales'].sum()
venta_bruta = df_filtrado[df_filtrado['Es_Devolucion'] == False]['Weekly_Sales'].sum()
pct_devolucion = (abs(devoluciones) / venta_bruta * 100) if venta_bruta else 0

# ==========================================================
# KPIs (título en inglés arriba, valor numérico dentro de la card)
# ==========================================================
c1, c2, c3, c4 = st.columns(4)
kpi_card(c1, "Total Period Sales", "kpi-yellow", f"${formato_k_mm(venta_total)}")
kpi_card(c2, "Weekly Avg Sales vs Last Holiday", "kpi-purple",
          f"${formato_k_mm(venta_promedio_periodo)}", f"{delta_vs_holiday:+.1f}%")
kpi_card(c3, "Top Store of Period", "kpi-green",
          f"Store {tienda_top}", f"${formato_k_mm(venta_tienda_top)}")
kpi_card(c4, "Returns / Gross Sales", "kpi-coral", f"{pct_devolucion:.1f}%")
